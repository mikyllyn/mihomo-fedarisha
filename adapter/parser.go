...
func ParseProxy(mapping map[string]any, options ...ProxyOption) (C.Proxy, error) {
	...
	case "fedarisha":
		fedarishaOption := &outbound.FedarishaOption{BasicOption: basicOption}
		err = decoder.Decode(mapping, fedarishaOption)
		if err != nil {
			break
		}
		proxy, err = outbound.NewFedarisha(*fedarishaOption)
	case "sudoku":
